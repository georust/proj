...
The `bundled_proj` feature allows you to link against a `libproj` included with (and built from source by) the `proj-sys` crate, upon which this crate is built. To do so, enable the `bundled_proj` Cargo feature. This feature requires sqlite3 to be present on your system.
